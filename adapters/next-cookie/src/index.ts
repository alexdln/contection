    ...
    async getServerSnapshot(store: Store) {
        ...
        if (this.storage) {
            for (const key of keys) {
                const savedValue = await this.readFromServerStorage(key);
                if (savedValue) {
